...
# In large_grid/data/build_file.py

# ... keep existing imports ...

def output_flows(peak_flow1, peak_flow2, density, seed=None):
    # REPLACE the entire body of this function with just this:
    str_flows = '<routes>\n'
...
def output_config(thread=None):
    if thread is None:
        ...
        out_file = 'expdummy_%d.rou.xml' % int(thread)
    str_config = '<configuration>\n  <input>\n'
    str_config += '    <net-file value="exp.net.xml"/>\n'
    str_config += '    <route-files value="%s"/>\n' % out_file
    str_config += '    <additional-files value="exp.add.xml"/>\n'
...
def output_tls(tls, phase):
    ...
    str_adds += '</additional>\n'
    return str_adds
def output_dummy_flows():
    """
    Generates a route file with ONLY vType definitions and no flows.
    Used when spawning vehicles dynamically via TraCI.
    """
    str_flows = '<routes>\n'
    str_flows += '  <vType id="type1" length="5" accel="5" decel="10"/>\n'
    str_flows += '</routes>\n'
...
def main():
    ...
    write_file('./expdummy.rou.xml', output_dummy_flows())
