...
def train(args):
    ...
    # Pass dirs['model'] as model_path
    trainer = Trainer(env, model, global_counter, summary_writer, in_test, output_path=dirs['data'], model_path=dirs['model'])
    trainer.run()
...
def evaluate_fn(agent_dir, output_dir, seeds, port, demo, policy_type):
    ...
    # 1. Load config file FIRST to identify the real agent type
    ...
    logging.info(f"Evaluation: Processing folder '{agent_folder_name}' as agent type '{agent}'")

    # 2. Create a fresh Graph for this thread (Fixes "Key not found" & threading bugs)
