...
class CNNA2C(A2C):
    def __init__(self, n_s, n_a, total_step, model_config, seed=0, features_per_node=8):
        self.features_per_node = features_per_node
        super().__init__(n_s, n_a, total_step, model_config, seed=seed)

    def _init_policy(self, n_s, n_a, n_f, model_config, agent_name=None):
        # Use the new Continuous Gaussian Policy
        n_step = model_config.getint('batch_size') if not hasattr(self, 'n_step') else self.n_step
        return GaussianCNNACPolicy(n_s, n_a, n_step, name=agent_name)
    ...
    def backward(self, R, summary_writer=None, global_step=None):
        ...
        # --- FIX START ---
        # Ensure Advantage and Returns are 1D vectors
        # If the shape is (batch_size, 300), we average across agents to get a global metric
        if len(Advs.shape) > 1:
            Advs = np.mean(Advs, axis=1)
        if len(Rs.shape) > 1:
            Rs = np.mean(Rs, axis=1)
            
        # Ensure they are flattened to (?,) to match the placeholder
        Advs = Advs.flatten()
        Rs = Rs.flatten()
        # --- FIX END ---
# --- NEW: Log Training Metrics ---
        if summary_writer is not None:
            summary = tf.Summary()
            summary.value.add(tag='Train/Return_Mean', simple_value=np.mean(Rs))
            summary.value.add(tag='Train/Advantage_Mean', simple_value=np.mean(Advs))
            summary.value.add(tag='Train/Learning_Rate', simple_value=cur_lr)
            summary.value.add(tag='Train/Entropy_Coef', simple_value=cur_beta)
            
            # If gradients are calculated in policy.backward, they might be logged there.
            # But these high-level stats are very useful to confirm training is active.
            summary_writer.add_summary(summary, global_step)
        self.policy.backward(self.sess, obs, acts, dones, Rs, Advs, cur_lr, cur_beta,
                             summary_writer=summary_writer, global_step=global_step)
# --- UPDATE IN agents/models.py ---

class GCNA2C(A2C):
    def __init__(self, n_s, n_a, total_step, model_config, adj_matrix, num_nodes, feat_dim, seed=0):
        self.adj_matrix = adj_matrix
        self.num_nodes = num_nodes
        self.feat_dim = feat_dim
        super().__init__(n_s, n_a, total_step, model_config, seed)

    def _init_policy(self, n_s, n_a, n_f, model_config, agent_name=None):
        return GaussianGCNACPolicy(n_s, n_a, self.n_step, 
                                   self.adj_matrix, self.num_nodes, self.feat_dim, 
                                   name=agent_name)
    # ... (Rest is same as CNNA2C) ...
    def backward(self, R, summary_writer=None, global_step=None):
        cur_lr = self.lr_scheduler.get(self.n_step)
        cur_beta = self.beta_scheduler.get(self.n_step)
        obs, acts, dones, Rs, Advs = self.trans_buffer.sample_transition(R)
