"""
Evaluate baseline and retrained traffic-signal controllers on real-net demand groups.

This script mirrors eval_signal_controllers.py but targets the Monaco subnet
(`real_net`) demand groups and checkpoints.
"""
...
DEFAULT_DEMAND_DIR = os.path.join(PROJECT_ROOT, "real_net_subnet", "demand_groups")
DEFAULT_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "runs_eval", "signal_controller_benchmark_real")
BASE_GROUP_BLOCK_SEC = 600
DEFAULT_NUM_ROLLOUTS = 10
DEFAULT_GROUP_DURATION_SEC = 3600
DEMAND_REQUIRED_COLUMNS = {"origin_edge", "dest_edge", "veh_per_hour"}
DEMAND_GROUP_ORDER = [
    "N_to_S",
    "S_to_N",
    "W_to_E",
...
# Optional demand groups that should be included when present, after core groups.
DEMAND_OPTIONAL_GROUP_ORDER = [
    "Real_Life_Monaco",
]
...
ACTIVE_CONTROLLERS: List[ControllerSpec] = [
    ControllerSpec(
        ...
        family="ia2c",
        variant="baseline",
        model_dir=os.path.join(PROJECT_ROOT, "runs", "ia2c_real", "model"),
        config_hint=os.path.join(PROJECT_ROOT, "runs", "ia2c_real", "data", "config_ia2c_real.ini"),
    ),
    ControllerSpec(
        name="ma2c_real",
        label="MA2C Real",
        family="ma2c",
        variant="baseline",
        model_dir=os.path.join(PROJECT_ROOT, "runs", "ma2c_real", "model"),
        config_hint=os.path.join(PROJECT_ROOT, "runs", "ma2c_real", "data", "config_ma2c_real.ini"),
    ),
    ControllerSpec(
        name="ppo_real",
        label="PPO Real",
        family="ppo",
        variant="baseline",
        model_dir=os.path.join(PROJECT_ROOT, "runs", "ppo_real", "model"),
        config_hint=os.path.join(PROJECT_ROOT, "runs", "ppo_real", "data", "config_ppo_real.ini"),
    ),
    ControllerSpec(
        name="iqll_real",
        label="IQL-LR Real",
        family="iqll",
        variant="baseline",
        model_dir=os.path.join(PROJECT_ROOT, "runs", "iqll_real", "model"),
        config_hint=os.path.join(PROJECT_ROOT, "config", "config_iqll_real.ini"),
    ),
    ControllerSpec(
        name="ia2c_retrained",
        ...
        family="ia2c",
        variant="retrained",
        model_dir=os.path.join(PROJECT_ROOT, "output_coevolution_real", "ia2c_from_real_fresh", "model_traffic"),
        config_hint=os.path.join(PROJECT_ROOT, "config", "config_ia2c_real.ini"),
    ),
    ControllerSpec(
        name="ma2c_retrained",
        ...
        family="ma2c",
        variant="retrained",
        model_dir=os.path.join(PROJECT_ROOT, "output_coevolution_real", "ma2c_real_old", "model_traffic"),
        config_hint=os.path.join(PROJECT_ROOT, "config", "config_ma2c_real.ini"),
    ),
    ControllerSpec(
        name="ppo_retrained",
        label="PPO Retrained",
        family="ppo",
        variant="retrained",
        model_dir=os.path.join(PROJECT_ROOT, "output_coevolution_real", "ppo", "model_traffic"),
        config_hint=os.path.join(PROJECT_ROOT, "config", "config_ppo_real.ini"),
    ),
    ControllerSpec(
        name="iqll_retrained",
        ...
        family="iqll",
        variant="retrained",
        model_dir=os.path.join(PROJECT_ROOT, "output_coevolution_real", "iql-lr", "model_traffic"),
        config_hint=os.path.join(PROJECT_ROOT, "config", "config_iqll_real.ini"),
    ),
]
...
COMPARISON_PAIRS: List[Tuple[str, str]] = [
    ("ia2c_real", "ia2c_retrained"),
    ("ma2c_real", "ma2c_retrained"),
    ("ppo_real", "ppo_retrained"),
    ("iqll_real", "iqll_retrained"),
]


class DemandSubsetRealNetEnv(RealNetEnv):
    """Real-net env that only loads the provided demand CSVs."""
    ...
    def _resolve_demand_group_dir(self):
        ...
        return super()._resolve_demand_group_dir()

    def _load_scenarios(self):
        if not self._demand_csvs:
            return super()._load_scenarios()
        ...
        for i, path in enumerate(self.csv_paths):
            ...
            if missing_cols:
                raise ValueError(
                    "Demand CSV missing required column(s) %s: %s"
                    % (", ".join(missing_cols), path)
                )
...
def resolve_demand_groups(demand_dir: str) -> List[Tuple[int, str, str]]:
    ...
    for path in csv_paths:
        ...
        if not DEMAND_REQUIRED_COLUMNS.issubset(fieldnames):
            logging.info(
                "Skipping non-demand CSV %s (missing columns: %s)",
                os.path.basename(path),
                ", ".join(sorted(DEMAND_REQUIRED_COLUMNS - fieldnames)),
            )
            continue
        path_by_name[os.path.splitext(os.path.basename(path))[0]] = path

    if not path_by_name:
        raise FileNotFoundError(
            "No demand CSV files with required columns (%s) found under %s"
            % (", ".join(sorted(DEMAND_REQUIRED_COLUMNS)), os.path.abspath(demand_dir))
        )
    ...
    optional_names_present = [
        name for name in DEMAND_OPTIONAL_GROUP_ORDER if name in path_by_name
    ]
    ordered_names.extend(optional_names_present)

    extra_names = sorted(
        name
        for name in path_by_name
        if name not in DEMAND_GROUP_ORDER and name not in DEMAND_OPTIONAL_GROUP_ORDER
    )
...
def resolve_config_path(spec: ControllerSpec) -> str:
    ...
    fallback_run_config = os.path.join(
        PROJECT_ROOT,
        "config",
        "config_%s_real.ini" % spec.family,
    )
    if fallback_run_config not in candidates:
        candidates.append(fallback_run_config)
    fallback_resume_config = os.path.join(
        PROJECT_ROOT,
        "config",
        "config_%s_real_resume.ini" % spec.family,
    )
    if fallback_resume_config not in candidates:
        candidates.append(fallback_resume_config)
...
def evaluate_controller(
    ...
    try:
        for group_offset, (group_index, group_name, demand_csv) in enumerate(demand_groups):
            demand_sequence = build_group_demand_sequence(demand_csv, group_duration_sec)
            logging.info(
                "Evaluating controller=%s on group=%d (%s), duration=%ds (%d x %ds blocks)",
                spec.name,
                group_index,
                group_name,
                group_duration_sec,
